...
if (!class_exists('RWMB_Select_Advanced_Field')) {
    class RWMB_Select_Advanced_Field extends RWMB_Select_Field {
        ...
        static function html($html, $meta, $field) {
            $html = sprintf('<select class="rwmb-select-advanced" name="%s" id="%s"%s data-options="%s">', $field['field_name'], $field['id'], $field['multiple'] ? ' multiple="multiple"' : '', esc_attr(json_encode($field['js_options'])));
            if (!empty($field['js_options']['placeholder']))
                $html .= '<option></option>';
            $option = '<option value="%s" %s>%s</option>';
            //that is for each field selection
            //
            foreach ($field['options'] as $value => $label) {
                //recent modifications
                $html .= sprintf($option, $value, selected(($value==$meta), true, false), $label);
            }
            $html .= '</select>';
